...
@WebServlet(name = "TopGenresServlet", urlPatterns = "/top_genres")
public class TopGenresServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<Integer> genresFromPools = new ArrayList<>();

        for (SavedPoll savedPoll : savedPolls) {
            for (int genre : savedPoll.getPool().getGenres()) {
                genresFromPools.add(genre);
            }
        }

        Map<String, Integer> genresHashMap = new HashMap<>();

        for (int i = 0; i < service.getGenres().size(); i++) {
            genresHashMap.put(service.getGenres().get(i), Collections.frequency(genresFromPools, i + 1));
        }

        genresHashMap.entrySet()
                .stream()
